#!/usr/bin/env python3
"""
☕ نظام كافية الواحة - نسخة Back4App ☕

المميزات:
- شبكة واي فاي مفتوحة
- صفحة تسجيل الدخول بالكروت
...
# قاعدة البيانات (في tmp عشان نضمن الكتابة)
...
def login():
    ...
    if card:
        ...
        <body>
            <div class="success">✅ تم تفعيل الكرت {card_number}</div>
            <div>الوقت المتبقي: {minutes} دقيقة</div>
            <div class="timer" id="timer"></div>
            <div>جاري تحويلك إلى الإنترنت...</div>
            <script>
                ...
                function updateTimer() {{
                    ...
                    timerDiv.innerHTML = String(mins).padStart(2,'0') + ':' + String(secs).padStart(2,'0');
                    if(remaining > 0) {{
                        remaining--;
...
def admin_login():
    if request.method == 'POST':
        ...
        if username == ADMIN_USERNAME and hash_password(password) == ADMIN_PASSWORD_HASH:
            ...
            <body style="background:#0a0a0a;color:#ff0033;text-align:center;padding:50px;font-family:sans-serif">
                <h1>❌ خطأ في الدخول</h1>
...
def admin_cards():
    ...
    cards = get_all_cards()
    available = sum(1 for c in cards if not c[4])
    used = sum(1 for c in cards if c[4])
    ...
    <body>
        ...
        <div class="stats">
            ...
            <strong style="color:#ff6b35">📊 الإجمالي:</strong> <span style="color:#fff">{len(cards)}</span>
        </div>
        <table>
...
def admin_add():
    ...
    if add_card(card_number, minutes):
        ...
        <html>
        <head><meta charset="UTF-8"></head>
        <body style="background:#0a0a0a;color:#00ff41;text-align:center;padding:50px;font-family:sans-serif">
            <h1>✅ تم إضافة الكرت {card_number}</h1>
            <p>عدد الدقائق: {minutes}</p>
            <a href="/admin" style="color:#ff6b35">العودة للوحة</a>
        </body>
        </html>
        '''
    else:
