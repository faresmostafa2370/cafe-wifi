...
المميزات:
- تفعيل بالبيانات (ميجابايت) بدلاً من الوقت
- صفحة مستخدم جذابة وبسيطة
- لوحة تحكم أدمن متكاملة وجميلة
- إدارة كروت بالرصيد (ميجابايت)
    ...
    <div class="container">
        ...
        <div class="prices">
            <div class="price-card">
                <div class="hours">1 ساعة</div>
                <div class="data">500 ميجابايت</div>
                <div class="price">10 ج</div>
            </div>
            <div class="price-card">
                <div class="hours">2 ساعة</div>
                <div class="data">1 جيجابايت</div>
                <div class="price">15 ج</div>
            </div>
            <div class="price-card">
                <div class="hours">4 ساعة</div>
                <div class="data">2 جيجابايت</div>
                <div class="price">25 ج</div>
            </div>
            <div class="price-card">
                <div class="hours">8 ساعة</div>
                <div class="data">4 جيجابايت</div>
                <div class="price">40 ج</div>
            </div>
        </div>
        <div class="footer">
            <div class="contact">📞 للكروت: {{ service_number }}</div>
            <div>⚡ سرعات عالية | 📡 تغطية شاملة</div>
        </div>
    </div>
</body>
</html>
    ...
    <div class="container">
        ...
        <div class="stats">
            <div class="stat-card">
                ...
                <div class="data-info">{{ remaining_percent }}% من الرصيد</div>
            </div>
            <div class="stat-card">
        ...
        <div class="progress-bar">
            <div class="progress-fill" id="progress-fill" style="width: {{ remaining_percent }}%"></div>
        </div>
        
        <div class="info">
            <p>📡 عنوان الأيبي: {{ ip }}</p>
            <p>⏱️ وقت البدء: {{ start_time }}</p>
            <p>⏰ ينتهي في: {{ end_time }}</p>
        </div>
        ...
        <div class="warning">⚠️ سيتم قطع الإنترنت تلقائياً عند انتهاء الرصيد أو الوقت</div>
    </div>
    
    <script>
        let remainingSeconds = {{ remaining_seconds }};
        let remainingData = {{ remaining_data }};
...
# ========== ADMIN DASHBOARD (جميلة ومتكاملة) ==========
ADMIN_LOGIN_PAGE = '''
<!DOCTYPE html>
<html lang="ar">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>دخول الأدمن - كافية الواحة</title>
    <link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;600;700&display=swap" rel="stylesheet">
    <style>
...
<body>
    <div class="container">
        ...
        <div class="menu">
            ...
            <a href="/admin/cards" class="btn">💳 إدارة الكروت</a>
            <a href="/admin/sessions" class="btn">🟢 الجلسات النشطة</a>
            <a href="/admin/logout" class="btn btn-danger">🚪 تسجيل الخروج</a>
        </div>
        <div class="card">
            ...
            <form method="POST" action="/admin/add">
                ...
                <button type="submit">➕ إضافة كرت</button>
            </form>
        </div>
        <div class="card">
            ...
            <div style="overflow-x:auto">
                <table>
                    <thead>
                        <tr><th>ID</th><th>رقم الكرت</th><th>الرصيد (MB)</th><th>تاريخ الإضافة</th><th>الحالة</th></tr>
                    </thead>
                    <tbody>
                        {{ cards_table }}
                    </tbody>
...
def update_session_remaining(session_id, remaining_data):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("UPDATE sessions SET remaining_data=? WHERE id=?", (remaining_data, session_id))
    conn.commit()
    conn.close()

def get_session_remaining(session_id):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT remaining_data FROM sessions WHERE id=?", (session_id,))
    result = c.fetchone()
    conn.close()
...
def login():
    ...
    if card:
        ...
        end_time = start_time + datetime.timedelta(hours=data_mb // 500)
        remaining_seconds = (end_time - start_time).seconds
        
        # حساب الوقت التقريبي (1 ساعة لكل 500 ميجابايت)
        hours = data_mb // 500
        if data_mb % 500 > 0:
            hours += 1
        ...
        remaining_percent = int((remaining_data / data_mb) * 100)
        ...
        return render_template_string(CAPTIVE_PAGE, cafe_name=CAFE_NAME, service_number=SERVICE_NUMBER, error="❌ رقم الكرت غير صالح أو مستخدم")
    ...
    <body>
        <div class="container">
            ...
            <div class="stats">
                ...
                <div>جلسة نشطة حالياً</div>
            </div>
            <div style="overflow-x:auto">
                <table>
                    <thead>
                        <tr><th>ID</th><th>رقم الكرت</th><th>IP</th><th>وقت البدء</th><th>الرصيد (MB)</th><th>المتبقي</th><th>الحالة</th></tr>
                    </thead>
                    <tbody>
    '''
...
def admin_add():
    ...
    if add_card(card_number, data_mb):
        ...
        <head>
            ...
            <style>
                body{{background:#0a0a0a;color:#00ff88;text-align:center;padding:50px;font-family:sans-serif}}
            </style>
        </head>
        <body>
            <h1>✅ تم إضافة الكرت {card_number}</h1>
            <p>الرصيد: {data_mb} ميجابايت</p>
            <p>جاري التحويل...</p>
        </body>
        </html>
        '''
        ...
        <head>
            ...
            <meta http-equiv="refresh" content="2;url=/admin">
            <style>
                body{{background:#0a0a0a;color:#ff0033;text-align:center;padding:50px;font-family:sans-serif}}
            </style>
        </head>
        <body>
            <h1>❌ الكرت {card_number} موجود بالفعل</h1>
            <p>جاري التحويل...</p>
        </body>
        </html>
        '''
